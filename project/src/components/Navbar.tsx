...
import { Dumbbell, Award, HeartPulse, User, Menu, X } from 'lucide-react';
  ...
  return (
    <nav className="bg-white/70 backdrop-blur-lg border-b border-indigo-100 sticky top-0 z-50 shadow-sm">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {/* Navigation Links */}
        <div className={`md:flex md:items-center md:space-x-8 ${isOpen ? 'block' : 'hidden'} md:block absolute md:static bg-white md:bg-transparent w-full md:w-auto top-16 left-0 p-4 md:p-0 shadow-md md:shadow-none ${isOpen ? 'flex-col space-y-4' : ''}`}>
          ...
          {currentUser ? (
            <Link to="/profile" className="flex items-center space-x-2 text-gray-600 hover:text-indigo-600 transition-colors duration-200 group">
              <div className="relative">
                <img src={currentUser.avatar} alt={currentUser.name} className="w-9 h-9 rounded-full object-cover border-2 border-indigo-100 group-hover:border-indigo-300 transition-colors duration-200" />
            ...
            <Link to="/login" className="flex items-center space-x-2 px-4 py-2 rounded-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-medium hover:from-indigo-700 hover:to-blue-700 transition-all duration-300 shadow-sm hover:shadow group">
